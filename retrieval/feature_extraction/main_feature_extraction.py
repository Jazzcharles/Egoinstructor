...
### add internvideo env ###
# sys.path.insert(0, '/mnt/petrelfs/xujilan/tools/InternVideo/Pretrain/Multi-Modalities-Pretraining')
# sys.path.insert(0, './models/InternVideo/Pretrain/Multi-Modalities-Pretraining')
from models.InternVideo.Pretrain.Multi_Modalities_Pretraining import InternVideo
from models.InternVideo.Pretrain.Multi_Modalities_Pretraining.InternVideo.clip_utils.utils.attention import MultiheadAttention

### add videomae ###
# sys.path.insert(0, '/mnt/petrelfs/xujilan/tools/retaug')
from models.retriever import FLIP_VideoMAEV2_LARGE_OPENAI
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        ...
        ### maybe also visual_proj ###
        for name in ["text_projection", "proj", "temporal_cls_token", "q_bias", "v_bias", 'image_projection', \
            'positional_embedding','cls_token', 'pos_embed','temporal_embed']:
            if hasattr(l, name): 
                ### if we have already changed some of which, do not change it again as it might not be attribute
                attr = getattr(l, name)
                if attr is not None and isinstance(attr, (nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.Linear)):
...
class HTM_LongLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        vlen = self.get_vlen_float(video_path)
        vlen = int(vlen) if vlen > 1 else vlen
    ...
    def _get_video_frame(self, video_path, vlen):
        ...
        total_num_frames = max(self.fps, vlen * self.fps)
        start = 0
        end = vlen
...
def extract_feature(loader, model, device, epoch, args):
    ...
    for idx, input_data in tqdm(enumerate(loader), total=len(loader)):
        ...
        for video_seq in tqdm(video_seq_list, total=len(video_seq_list), leave=False):
            if args.model.startswith('videomae'):
        ...
        print(f'all_v_features shape={all_v_features.shape}')
...
def main(args):
    ...
    if args.model in ['internvideo-ffmpeg']:
        model = InternVideo.load_model(args.exo_checkpoint).cuda()
        if args.half:
        ...
        model = FLIP_VideoMAEV2_LARGE_OPENAI(project_embed_dim=768)
        state = torch.load(args.ego_checkpoint, map_location='cpu')['state_dict']
        newstate = {}
        for k, v in state.items():
            newk = k.replace('module.','')
            newstate[newk] = v
        res = model.load_state_dict(newstate, strict=False)
        print(res)
        model = model.to(device)
        fps = 8
        feature_tag = 'VideoMAE_feature'

    ### Optional: process short video first, for easy debugging ###
    htm_vlen_df = get_htm_vlen_df(args.video_csv)
    htm_vid_to_vlen = dict(zip(htm_vlen_df['vid'], htm_vlen_df['vlen']))
    vid_list = sorted(glob.glob(os.path.join(args.video_root, '*')), key=lambda x: htm_vid_to_vlen.get(os.path.basename(x).split('.')[0], 10000))
    
    # vid_list = [os.path.join(args.video_root, i.strip()) for i in vid_list]
    if len(vid_list) == 0:
        print(f'no videos found in {args.video_root}, skip')
    ori_num_vid = len(vid_list)
    
    args.partname = os.path.basename(args.video_root)
    ...
    #     # args.output_dir = '/mnt/petrelfs/xujilan/VideoMAE_feature_fps8/gaze_video_features/'
    ...
    if len(vid_list) == 0:
        print(f'no videos need to do in {args.video_root}, skip')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--part', default='', nargs='*', type=str)
    parser.add_argument('--video_root', default='./assets/videos/', type=str)
    parser.add_argument('--video_csv', default='./assets/video_info.csv', type=str)
    parser.add_argument('--exo_checkpoint', default='./checkpoints/InternVideo-MM-L-14.ckpt', type=str)
    parser.add_argument('--ego_checkpoint', default='./checkpoints/videomae_L14_checkpoint_best.pth', type=str)
    parser.add_argument('--output_dir', default='./assets/video_features/', type=str)
