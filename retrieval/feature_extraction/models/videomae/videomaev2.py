...
from .videomaev2_modeling_finetune import (
    Block,
    PatchEmbed,
    _cfg,
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def inflate_temporal_embeds(self, x, num_frames):
        org_num_frames = 8
        T = org_num_frames // self.patch_embed.tubelet_size
        pos_tokens = self.pos_embed

        new_T = num_frames // self.patch_embed.tubelet_size
        ...
        self.pos_embed = pos_tokens

    def forward_features(self, x, mask):
        num_frames = x.shape[2] #[B,C,T,H,W]
        x = self.patch_embed(x)

        if mask is None:
            mask = torch.zeros((x.size(0), x.size(1))).to(x.device).bool()

        if x.shape[1] == self.pos_embed.shape[1]:
            pass 
        elif x.shape[1] < self.pos_embed.shape[1]:
            print(f'Encounting smaller input, chunking pos_embed from {self.pos_embed.shape[1]} to {x.shape[1]}')
        else:
            ### this is usually done at inference ###
            print(f'Require inflating position embedding from {self.pos_embed.shape[1]} to {x.shape[1]}')
            self.inflate_temporal_embeds(x, num_frames)
